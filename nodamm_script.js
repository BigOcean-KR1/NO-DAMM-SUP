...
function initGallerySlider() {
  ...
  slides.forEach((_, i) => {
    ...
    dot.style.cssText = `width:10px;height:10px;border-radius:50%;border:none;cursor:pointer;transition:all 0.3s;background:${i===0?'var(--green)':'#ccc'};padding:0;`;
  ...
  function updateDots() {
    dotsEl.querySelectorAll('button').forEach((d, i) => {
      d.style.background = i === current ? 'var(--green)' : '#ccc';
  ...
  window.addEventListener('mouseup', () => {
    ...
    if (dragDiff < -60) goTo(current + 1);
    else if (dragDiff > 60) goTo(current - 1);
    else goTo(current);
    dragDiff = 0;
  });
...
window.switchTab = function(tabName) {
  ...
  if (tabName === 'gallery') {
    setTimeout(initGallerySlider, 50);
  }
};
